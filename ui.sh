...
ui::pick() {
   ...
   local -r fzf_cmd="$([ $FM_ENV == "test" ] && echo "fzf_mock" || echo "fzf")"
...
ui::select() {
   ...
   local -r script_path="${SCRIPT_DIR}/frostmourne"
   ...
   if [ "$entry_point" = "search" ]; then
      args+=("--header"); args+=("Displaying online results. Please refer to 'frostmourne --help' for details")
   fi
